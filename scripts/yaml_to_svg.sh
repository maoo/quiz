...
# generate_svgs.sh - Script to generate SVG files from YAML questions
...
#   YAML_FILES: Optional. Space-separated list of YAML files to process.
#               If provided, this takes precedence over deck_name.

set -e  # Exit on error

# Function to get YAML files based on deck name
get_yaml_files() {
    local deck_name="$1"
    if [ -n "$YAML_FILES" ]; then
        echo "$YAML_FILES"
    elif [ -n "$deck_name" ]; then
        find "decks/$deck_name/questions" -name 'question.yaml'
    else
        find decks -name 'question.yaml'
    fi
}
...
print_usage() {
    ...
    echo "  YAML_FILES: Optional. Space-separated list of YAML files to process."
    echo "              If provided, this takes precedence over deck_name."
    echo ""
    echo "Example: $0 devops-hero"
    echo "         $0"
    echo "         YAML_FILES='decks/deck1/questions/001/question.yaml decks/deck2/questions/002/question.yaml' $0"
}
...
for file in $(get_yaml_files "$1"); do
    if [ -f "$file" ]; then
        output_file="$(dirname "$file")/card.svg"
        echo "Processing $file -> $output_file"
        poetry run python -m src.yaml_to_svg.generator "$file" "$output_file"
    fi
done
