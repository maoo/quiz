...
@router.post("/convert")
def convert_yaml_to_markdown_endpoint(request, data: dict = Body(...)):
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(answers_file, "w") as f:
                f.write("")
            # Create a minimal valid index.yaml so YAMLToMarkdown does not fail
            index_file = os.path.join(temp_dir, "index.yaml")
            with open(index_file, "w") as f:
                f.write("title: Test Quiz\n")
            converter = YAMLToMarkdown([temp_dir])
            converter.process_deck()
            md_file = os.path.join(cards_dir, "content.md")
